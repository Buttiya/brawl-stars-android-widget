...
Демо-версия: без централизованного API-слоя, обновление данных только вручную через кнопку `Refresh`.
...
1. Выделенный API-слой для семейств/сообщений/новостей.
...
3. Верификация и сообщения работают локально, без внешней инфраструктуры.

### 4.2 Production Target Architecture
1. `App API` для пользователей, клубов, семейств, сообщений и новостей.
...
1. Для демо принимается локальная модель доверия без отдельной криптографической верификации на стороне API.
2. Источники внешних данных по клубам и картам остаются доступными по текущему API-слою.
3. Продакшн API-слой будет добавлен отдельной фазой без критического рефакторинга UI-контрактов.
